    ...
    async executeStep(stepJson: StepJson, runtimeParams: string[]) {
        ...
        for (const action of stepJson.actions) {
            try {
                ...
                Logger.info(`Performing: ${action.actionType} - ${action.description || ''}${action.optional ? ' (Optional)' : ''}`, 'Executor');
                ...
                if (action.optional) {
                    Logger.warn(`Optional action failed: ${action.actionType} - ${action.description}. Skipping...`, 'Executor');
                } else {
                    throw error;
                }
            }
